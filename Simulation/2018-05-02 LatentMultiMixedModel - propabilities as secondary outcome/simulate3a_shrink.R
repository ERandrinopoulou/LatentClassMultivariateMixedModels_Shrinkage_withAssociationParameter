...
betaa1 <- c("(Intercept)" = -1.04674145, "Group1" = -0.70988616, "Time1" =  0.82395339)
sigma1.y <- 1.017671  # measurement error standard deviation

# parameters for the linear mixed effects model 
betaa2 <- c("(Intercept)" = 0.36239039, "Group1" = 0.50113490, "Time1" =  0.49765166)

# parameters for the logistic mixed effects model 
betaa3 <- c("(Intercept)" = -5.2947848, "Group1" = -0.9861937, "Time1" =  0.9005431)

# association parameter
alphaa1 <- -1.353078#-0.5 # association parameter - value
alphaa2 <- 0.5085359#-0.5 # association parameter - value
...
eta.y1 <- as.vector(X1 %*% betaa1 + rowSums(Z1 * b[id, 1:2])) + alphaa1 * exp(eta.y2)/(1+exp(eta.y2)) + alphaa2 * exp(eta.y3)/(1+exp(eta.y3)) # linear predictor
