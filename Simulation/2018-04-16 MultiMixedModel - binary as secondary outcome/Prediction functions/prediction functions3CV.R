...
predictY <- function(anadata, sims.list, fm_VA, fm_I, fm_Com, newdata, timesPred, M = 200L, 
  ...
  y3 <- newdata$y3[newdata$id %in% indpred]
  ...
  formYx_Com <- formula(noquote(paste("y3 ~", c(terms(fm_Com)[[3]]))))
  mfX_Com <- model.frame(terms(formYx_Com), data = anadata)
  TermsX3 <- attr(mfX_Com, "terms")
  formYz_Com <- formula(noquote(paste("~", attributes(terms(fm_Com))$term.labels[[1]]))) #~ ns(time, 3) #time
  mfZ_Com <- model.frame(terms(formYz_Com), data = anadata)
  TermsZ3 <- attr(mfZ_Com, "terms")
  ...
  mfXpred3 <- model.frame(TermsX3, data = newdata)
  mfZpred3 <- model.frame(TermsZ3, data = newdata)
  ...
  formYx3 <- reformulate(attr(delete.response(TermsX3), "term.labels"))
  formYz3 <- reformulate(attr(delete.response(TermsZ3), "term.labels"))
  ...
  X3 <- model.matrix(formYx3, mfXpred3)
  Z3 <- model.matrix(formYz3, mfZpred3)
  ncZ <- ncol(Z1)
  ncZ2 <- ncol(Z2)
  ncZ3 <- ncol(Z3)
  ncX <- ncol(X1)
  ...
  mfXpred3 <- model.frame(TermsX3, data = data.id2)
  mfZpred3 <- model.frame(TermsZ3, data = data.id2)
  ...
  Xpred3 <- model.matrix(formYx3, mfXpred3)
  Zpred3 <- model.matrix(formYz3, mfZpred3)
  ...
  betas_Com <- apply(sims.list[[3]][,1:ncol(X3)],2,mean) 

  
  sigma_y <- 1/sqrt(mean(sims.list[[4]])) #lmeObject$postMeans$sigma2
  alphas_y <- mean(sims.list[[1]][,(ncol(X1) + 1)])
  alphas_Com <- mean(sims.list[[1]][,(ncol(X1) + 2)])
  D <- solve(matrix(apply(sims.list[[5]],2,mean), ncol(Z1) + ncol(Z2) + ncol(Z3), ncol(Z1) + ncol(Z2) + ncol(Z3)))
  
  
  #id <- match(newdata[[idVar]], unique(newdata[[idVar]]))
  modes <- matrix(0, n.tp, ncol(Z1) + ncol(Z2) + ncol(Z3))
  ...
  for (v in seq_len(n.tp)) {
    ...
    betas.new_y <- betas_y
    betas.new_x <- betas_x
    betas.new_Com <- betas_Com
    sigma.new <- sigma_y
    D.new <- D
    alpha.new1 <- alphas_y
    alpha.new2 <- alphas_Com
    
    ff <- function(b, y_y, y_x, y_Com, X_new_y, Z_new_y, X_new_x, Z_new_x, X_new_Com, Z_new_Com, 
                   betas.new_y, betas.new_x, betas.new_Com, sigma.new, 
                   D.new, 
                   alpha.new1, alpha.new2) {
      - ( -(1/(2*sigma.new^2))*(t(y_y-X_new_y%*%betas.new_y-Z_new_y%*%b[(1:ncZ)] - 
                                    alpha.new1 * (X_new_x%*%betas.new_x + Z_new_x%*% b[(ncZ+1):(ncZ+ncZ2)]) - 
                                    alpha.new2 * (X_new_Com%*%betas.new_Com + Z_new_Com%*% b[(ncZ+ncZ2+1):(ncZ+ncZ2+ncZ3)])   )%*%
                                  (y_y-X_new_y%*%betas.new_y-Z_new_y%*%b[(1:ncZ)] 
                                   -alpha.new1 * (X_new_x%*%betas.new_x + Z_new_x%*% b[(ncZ+1):(ncZ+ncZ2)]) 
                                   -alpha.new2 * (X_new_Com%*%betas.new_Com + Z_new_Com%*% b[(ncZ+ncZ2+1):(ncZ+ncZ2+ncZ3)]) )  ) +
            sum(like(pii(X_new_x, betas.new_x, Z_new_x, b[(ncZ+1):(ncZ+ncZ2)]), y_x)) +
            sum(like(pii(X_new_Com, betas.new_Com, Z_new_Com, b[(ncZ+ncZ2+1):(ncZ+ncZ2+ncZ3)]), y_Com)) -
            (1/2*(t(b)%*%solve(D.new)%*%(b)))  )  }
    opt <- try(optim(rep(0, (ncol(Z1) + ncol(Z2) + ncol(Z3))), ff, y_y = y_y[], y_x = y_x[], y_Com = y_Com[], 
                     X_new_y = X1[,], Z_new_y = Z1[,], X_new_x = X2[,], Z_new_x = Z2[,], X_new_Com = X3[,], Z_new_Com = Z3[,],
                     betas.new_y = betas.new_y, betas.new_x = betas.new_x, betas.new_Com = betas.new_Com, sigma.new = sigma.new, 
                     D.new = D.new,
                     alpha.new1 = alpha.new1, alpha.new2 = alpha.new2, 
                     method = "BFGS", hessian = TRUE), TRUE)
    
    if (inherits(opt, "try-error")) {
      gg <- function(b, y_y, y_x, y_Com, X_new_y, Z_new_y, X_new_x, Z_new_x, X_new_Com, Z_new_Com, betas.new_y, betas.new_x, betas.new_Com, sigma.new, 
                     D.new, 
                     alpha.new1, alpha.new2) {cd(b, ff, y_y = y_y, y_x = y_x, y_Com = y_Com, 
                                    X_new_y = X1[,], Z_new_y = Z1[,], X_new_x = X2[,], Z_new_x = Z2[,], X_new_Com = X3[,], Z_new_Com = Z3[,],
                                    betas.new_y = betas.new_y, betas.new_x = betas.new_x, betas.new_Com = betas.new_Com, sigma.new = sigma.new, 
                                    D.new = D.new,
                                    alpha.new1 = alpha.new1, alpha.new2 = alpha.new2)}
      opt <- optim(rep(0, (ncol(Z1) + ncol(Z2) + ncol(Z3))), ff, gg, y_y = y_y, y_x = y_x, y_Com = y_Com,
                   X_new_y = X1[,], Z_new_y = Z1[,], X_new_x = X2[,], Z_new_x = Z2[,], X_new_Com = X3[,], Z_new_Com = Z3[,],
                   betas.new_y = betas.new_y, betas.new_x = betas.new_x, betas.new_Com = betas.new_Com, sigma.new = sigma.new, 
                   D.new = D.new,
                   alpha.new1 = alpha.new1, alpha.new2 = alpha.new2,  
  ...
  ypred3 <- as.vector(c(Xpred3 %*% betas_Com) + rowSums(Zpred3 * matrix(rep(modes[(ncZ + ncZ2 + 1):(ncZ + ncZ2 + ncZ3)], each = nrow(Zpred3)), ,
                                                                        ncol(Zpred3))))

  y22 <- rbinom(length(ypred2), 1, exp(ypred2)/(1+exp(ypred2)))
  y33 <- rbinom(length(ypred3), 1, exp(ypred3)/(1+exp(ypred3)))
  ypred1 <- as.vector(c(Xpred1 %*% betas_y) + rowSums(Zpred1 * matrix(rep(modes[(1):(ncZ)], each = nrow(Zpred1)), ,ncol(Zpred1))) +
                        alphas_y * y22 + alphas_Com * y3  )
  
  ########## Output
  out <- data.frame(ypred1 = unlist(ypred1), 
                    ypredIN = unlist(ypred2), 
                    ypredCom = unlist(ypred3), 
