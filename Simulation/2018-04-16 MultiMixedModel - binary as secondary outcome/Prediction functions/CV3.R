...
###### Sources
source(file="prediction functions3CV.R")
...
CVmodel <- function(ind, ttimes) {
  ...
  ###### fit the model
  source("analysis3.R", local = T)
  
  model.fit <- jags.model(file = "multiMixed3", data = Data, n.chains = con$n.chains, 
  ...
  for (k in 1:length(ind)){
    ...
    if(  any(ftimes > ttimes) & !any(is.na(patdata[[k]]$y3 == T)) & !any(is.na(patdata[[k]]$year) == T) & !any(is.na(patdata[[k]]$drug) == T)  ){
      ...
      (pred[[k]] <- predictY(anadata = data, sims.list = sims.list, fm_VA = fm_VA, fm_I = fm_I, fm_Com = fm_Com, newdata = patdata[[k]], timesPred = stimes, 
  ...
  for (k in 1:length(pred)){
    if (!is.null(pred[[k]])) {
      ...
      vaReal <- sumPat[sumPat$year %in% c(stimes), c("id", "year", "y1", "y2", "y3")]
      colnames(vaReal) <- c("ideye", "year", "y1", "y2", "y3")
      
      vaPred <- predData[predData$times.to.pred %in% c(stimes), c("ideye", "times.to.pred", "VApred", "ypred1", "ypredIN")] 
      colnames(vaPred) <- c("ideye", "year", "vaPred", "yPred", "INpred")
      dat <- merge(vaPred, vaReal, by = c("ideye", "year"))
      dat$ttime <- testtimes
      #if (dat$vaPred < 0) dat$vaPred <- 0
      dat$diff <-  if (any(dat$vaPred == 0) | any(backtransva(dat$y1) == 0)){
        1*((-log10(dat$vaPred + 0.001) + log10(backtransva(dat$y1) + 0.001)))
      } else {
        1*((-log10(dat$vaPred) + log10(backtransva(dat$y1))))
      }
      res[[k]] <- dat
    }
  }
