...
fm_Com <- glmer(complication ~ ageelder + time + (time | ideye), data = anadata, 
              family = binomial, 
              na.action = na.exclude,  control=glmerControl(optimizer = "bobyqa",
                                                            optCtrl = list(maxfun = 100000)))
...
###
formYx3 <- formula(noquote(paste("complication ~", c(terms(fm_Com)[[3]])))) #formula(lmeObject2)
TermsX3 <-  terms(lmeObject3)
mfX3 <- model.frame(TermsX3, data = anadata)
X3 <- model.matrix(formYx3, mfX3)

formYz3 <- formula(noquote(paste("~", attributes(terms(fm_Com))$term.labels[[1]]))) #~ ns(time, 3) #time
mfZ3 <- model.frame(terms(formYz3), data = anadata)
...
Data <- list(N = nY, offset = offset, X = X, 
             ...
             priorMean.alphas1 = alphas1,
             priorTau.alphas1 = 1/var.alphas1,
             
             priorMean.alphas2 = alphas2,
             priorTau.alphas2 = 1/var.alphas2,
             
             priorR.D = diag(1,(ncZ+ncZ2+ncZ3)), priorK.D = (ncZ+ncZ2+ncZ3)
             
)

parms <- c("betas1","betas2", "betas3", "tau", "inv.D", "b", "alphas1", "alphas2")
#####################
model.fit <- jags.model(file = "multiMixed3outcomes.txt", data = Data, n.chains = con$n.chains, 
